import redmemberService from '../services/redmemberService'
import Util from '../utils/Utils'

const util=new Util();

class redmemberController{
    ...
    static async create(req,res){
        ...
        try{
            const item = await redmemberService.create(data)
    ...
    static async retrieveByFlatId(req,res){
        ...
        const {id} = req.params;
        try{
            const items = await redmemberService.retrieveByFlatId(id)
            if(items){
    ...
    static async update(req,res){
        ...
        try{
            const item = await redmemberService.update(id,data)
            if(item[0]==1){
    ...
    static async delete(req,res){
        ...
        try{
            const item = await redmemberService.delete(id)
            if(item==1){
                util.setSuccess(200,"updated case")
                return util.send(res)
            }
...
export default redmemberController;