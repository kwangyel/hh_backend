...
class memberController{
    ...
    static async retrieveWithHouseholdCID(req,res){
        const {cid} = req.params
        util.setData(null)

        try{
            const item = await memberService.retrieveWithHouseholdCID(cid)
            if(item){
                util.setSuccess(200,"retrieved")
                util.setData(item)
                return util.send(res)
    ...
    static async retrieveWithOwnerCID(req,res){
        const {cid} = req.params
        util.setData(null)

        try{
            const item = await memberService.retrieveWithOwnerCID(cid)
            if(item){
                util.setSuccess(200,"retrieved")
                util.setData(item)
                return util.send(res)
    ...
    static async retrieveWithOwnerContact(req,res){
        const {contact} = req.params
        util.setData(null)

        try{
            const item = await memberService.retrieveWithOwnerContact(contact)
            if(item){
                util.setSuccess(200,"retrieved")
                util.setData(item)
                return util.send(res)
